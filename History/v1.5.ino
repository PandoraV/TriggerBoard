...
//  Edition 2021/04/12 v1.5
//  Copyright © 2020年 CIEE. All rights reserved.
//
//  提供最高50Hz的分频版
//  触发沿问题尚未解决，水中放电已验证无影响，营养液设备也未出问题。
...
unsigned long trig_period = 200; // 触发信号脉宽为200μs
unsigned long voltage_Zero_Judge_Period = 1; // 过零点信号脉宽长达暂定1ms(无用)
...
void frequency_division()
{
    ...
    if (!leap) // leap为true即num_count为work_state的整倍数时，使能开启
    {
        if (trig_EN == HIGH)
        {
            trig_EN = LOW;
...
void loop()
{
    ...
    digitalWrite(Trig, LOW);
    digitalWrite(WDI, LOW);
    digitalWrite(LED4, HIGH); // 正常时LED4亮，5灭
